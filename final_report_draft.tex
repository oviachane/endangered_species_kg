...
\subsection{Latent Semantic Learning Analysis}
A major success of this project is the demonstration that Knowledge Graph Embeddings (KGE) can capture "common sense" or latent semantic proximity even with limited training epochs. Using our custom \texttt{visualize\_embeddings.py} tool, we extracted the Nearest Neighbors for key entities in the high-dimensional vector space:

\begin{itemize}
    \item \textbf{Target: Giant Panda (wd:Q242)} $\rightarrow$ Nearest: \textit{Republic of China} (Native Region), \textit{Animal Habitat} (Domain Context), and \textit{List of Endangered Species} (Taxonomic Match).
    \item \textbf{Target: China (wd:Q148)} $\rightarrow$ Nearest: \textit{State in Asia} (Geographic Reality).
\end{itemize}

This proves that the TransE model successfully placed related entities close together in the embedding space. To visualize this clustering, we generated a t-SNE (t-Distributed Stochastic Neighbor Embedding) projection of 1,000 sampled entities.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{kg_artifacts/embeddings_tsne.png}
    \caption{t-SNE Projection of the Endangered Species KG (Placeholder). Clusters represent semantically similar entities identified by the TransE model.}
\end{figure}

Artifacts produced include binary model weights (\texttt{.pkl}), comprehensive metric logs (\texttt{results.json}), and the 2D latent space visualization.
